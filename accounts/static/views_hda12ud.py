...
def details(request, project_id):
    ...
    if request.user.is_authenticated:
        return render(request, 'accounts/details.html', {'t':t, 's':s, 'project': project, 'workers':workers, 'json1':json1, 'jsondep':jsondep ,'jsoninit':jsoninit, 'jsonfinish':jsonfinish, 'project':project, 'initdate':initdate, 'finishdate':finishdate, 'taskslist':taskslist, 'workers_no':workers_no, 'files_no':files_no, 'tasks_no':tasks_no})
    else:
        return redirect('/profiles/login/')    
...
def files(request, file_id, project_id):
    ...
    if request.user.is_authenticated:
        return render(request, 'accounts/files.html', {'t':t, 's':s,'files': files,'project':project,'content':content})
    else:
        return redirect('/profiles/login/')    
...
def base(request):
    t = Task.objects.filter(new_task=True)
    s = Suggestion.objects.filter(new_suggestion=True)
    return render(request, 'accounts/base.html', {'t':t, 's':s})
...
def profile_deployments(request, project_id):
    ...
    if (is_member(user)) and (user==project.deployer) and (project.deployed==False):
        ...
        return render(request, 'accounts/profdep.html', {'t':t, 's':s,'user':user,'project':project,'profile':profile})
    else:
...
def successful(request):
    ...
    if request.user.is_authenticated:
        return render(request, 'accounts/successful.html', {'t':t, 's':s, 'deployments':deployments, 'lst':lst, 'succ':succ, 'result':result,'profile':profile,'ranking':ranking, 'viewer_ranking':viewer_ranking})
    else:
        return redirect('/profiles/login/')    
...
def create_project(request):
   ...
   if request.method == "POST":
        ...
        form = CreateProject()
        return render(request, 'accounts/create_project.html', {'t':t, 's':s, 'form': form})
...
def edit_project(request,project_id):
    ...
    return render(request, 'accounts/edit_project.html', {'t':t, 's':s, 'form': form})
...
def delete_project(request,project_id):
    ...
    if (is_member(user)) and (user==project.deployer):
        form = CreateProject(request.POST, instance=project)
        project.delete()
        return render(request, 'accounts/deleted.html', {'t':t, 's':s, 'form': form})
    else:
        return HttpResponse("Permission denied!")
...
def deletef(request,file_id,project_id):
    ...
    return render(request,'accounts/deletef.html',{'t':t, 's':s, 'file': file,'project':project})

def deletep(request,project_id):
    t = Task.objects.filter(new_task=True)
    s = Suggestion.objects.filter(new_suggestion=True)
    project = Project.objects.get(pk=project_id)
    return render(request,'accounts/deletep.html',{'t':t, 's':s, 'project':project})
...
def delete_file(request,file_id,project_id):
    ...
    if (is_member(user)) and (user==project.deployer):
        ...
        file.delete()
        return redirect("/home/%d" %project_id)    
        return render(request, 'accounts/deleted.html', {'t':t, 's':s, 'form': form,'project':project})
    else:
        return HttpResponse("Permission denied!")
...
def create_file(request,project_id):
    ...
    if request.method == "POST":
        ...
        form = CreateFile()
    return render(request, 'accounts/create_file.html', {'t':t, 's':s, 'form': form,'proje':proje})   
...
def create_suggestion(request,project_id,file_id):
    ...
    return render(request, 'accounts/create_suggestion.html', {'t':t, 's':s, 'form': form,'project':project,'file':file})   
...
def view_suggestions(request,project_id,file_id):
    ...
    try:
        ...
        raise Http404("File does not exist")
    return render(request, 'accounts/view_suggestions.html', {'t':t, 's':s, 'project': project,'file':file})
...
def suggestions(request,project_id,file_id,suggestion_id):
    ...
    try:
        ...
        raise Http404("File does not exist")
    return render(request, 'accounts/suggestions.html', {'t':t, 's':s, 'file': file,'project':project,'suggestions':suggestions,'content':content})
...
def confirm_suggestion(request,project_id,file_id,suggestion_id):
    ...
    if request.user == project.deployer:
        if suggestions.confirmed == True:
            ...
            profile.save()
            suggestions.save()
            return render(request, 'accounts/confirm_suggestion.html',{'t':t, 's':s, 'file': file,'project':project,'suggestions':suggestions})
    else:
        return HttpResponse("Permission denied!")
...
def myprojects(request):
    ...
    user = request.user
    if request.user.is_authenticated:
        return render(request, 'accounts/myprojects.html', {'t':t, 's':s, 'user':user})
    else:
        return redirect('/profiles/login/')   
...
def accepting_sugg(request):
    ...
    if request.user.is_authenticated:
        return render(request, 'accounts/accepting_sugg.html', {'t':t, 's':s, 'projects':projects})
    else:
        return redirect('/profiles/login/')  
...
def notifications(request):
    ...
    return render(request, 'accounts/notifications.html', {'t':t, 's':s, 'projects':projects, 'tasks':tasks, 'task':task}) 
...
def profiles(request, name):
    ...
    return render(request, 'accounts/profiles.html', {'t':t, 's':s, 'user':user, 'taskslist':taskslist})
...
def confirm_request(request,name):
    ...
    user.save()
    return render(request, 'accounts/confirm_request.html', {'t':t, 's':s, 'user':user})

def request(request, name):
    t = Task.objects.filter(new_task=True)
    s = Suggestion.objects.filter(new_suggestion=True)
    user = User.objects.get(username=name)
    return render(request, 'accounts/request.html', {'t':t, 's':s, 'user':user})
...
def deployer_confirm(request):
    ...
    return render(request,'accounts/deployer.html', {'t':t, 's':s, 'user':user})
...
def find_deployer(request):
    ...
    projects = Project.objects.all()
    return render(request, 'accounts/find_deployer.html', {'t':t, 's':s, 'users':users,'projects':projects})  
      
def subject(request):
    t = Task.objects.filter(new_task=True)
    s = Suggestion.objects.filter(new_suggestion=True)
    return render(request, 'accounts/subject.html', {'t':t, 's':s, 'subject':subject})
...
def update(request, project_id):
    ...
    return render(request, 'accounts/update.html', {'t':t, 's':s, 'project1':project1, 'lst':lst})

def history(request, project_id):
    t = Task.objects.filter(new_task=True)
    s = Suggestion.objects.filter(new_suggestion=True)
    project = Project.objects.get(pk=project_id)
    return render(request, 'accounts/history.html', {'t':t, 's':s, 'project':project})
...
def goback(request,project_id):
    ...
    project.save()
    return redirect("/home/%d" %project_id)
    return render(request, 'accounts/goback.html', {'t':t, 's':s, 'project':project})
...
def graph(request):
    ...
    return render(request, 'accounts/graph.html', {'t':t, 's':s, 'graph': cht})
...
def create_task(request, project_id):
    ...
    if(user.is_superuser or user == project.deployer):
        ...
        project.save()
    return render(request, 'accounts/tasks.html', {'t':t, 's':s, 'form':form})
...
def create_task1(request,project_id, task_id):
    ...
    if(user.is_superuser or user == project.deployer):
        if request.method == "POST":
            ...
            form = NewTask()
    return render(request, 'accounts/create_task1.html', {'t':t, 's':s, 'profiles':profiles, 'form':form, 'url':url})
...
def create_task2(request,project_id, task_id):
    ...
    return render(request, 'accounts/create_task2.html', {'t':t, 's':s})
...
def tasks(request, project_id):
    ...
    return redirect('/home/%d' %project_id)
    return render(request, 'accounts/taskslist.html', {'t':t, 's':s, 'taskslist':taskslist})
...
def taskdetails(request, project_id, task_id):
    ...
    project = Project.objects.get(pk=project_id)
    task = Task.objects.get(pk=task_id)
    return render(request, 'accounts/taskdetails.html', {'t':t, 's':s, 'task':task, 'project':project})
...
def alltasks(request, project_id):
    ...
    jsonfinish = json.dumps(list(finishdate), cls=DjangoJSONEncoder)
    jsondep = json.dumps(list(deployer), cls=DjangoJSONEncoder)
    return render(request, 'accounts/alltasks.html', {'t':t, 's':s, 'json1':json1, 'jsondep':jsondep ,'jsoninit':jsoninit, 'jsonfinish':jsonfinish, 'project':project, 'initdate':initdate, 'finishdate':finishdate})
...
def finish_task(request, project_id, task_id):
    ...
    if (request.user == project.deployer or request.user.is_superuser):
        ...
        if task.finished == True:
            return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
    return render(request, 'accounts/finishtask.html', {'t':t, 's':s})
...
def add_deployers(request, project_id):
    ...
    project = Project.objects.get(pk=project_id)
    workers = project.workers.all()    
    return render(request, 'accounts/add_deployers.html', {'t':t, 's':s, 'profiles':profiles, 'workers':workers, 'project':project})
...
def add(request, project_id, name):
    ...
    return render(request, 'accounts/add.html', {'t':t, 's':s, 'profiles':profiles, 'deployer':deployer})
...
def tasks1(request):
    ...
    jsonfinish = json.dumps(list(finishdate), cls=DjangoJSONEncoder)
    jsondep = json.dumps(list(deployer), cls=DjangoJSONEncoder)
    return render(request, 'accounts/task1.html', {'t':t, 's':s, 'json1':json1, 'jsondep':jsondep ,'jsoninit':jsoninit, 'jsonfinish':jsonfinish, 'initdate':initdate, 'finishdate':finishdate})
...
def profs(request):
    ...
    return render(request, 'accounts/profs.html', {'t':t, 's':s, 'profiles':profiles})
...
def pay(request, name):
    ...
    profile.save()
    return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
    return render(request, "accounts/payment.html", {'t':t, 's':s})
...
def create_todo(request,project_id, task_id):
    ...
    task.save()
    return render(request, 'accounts/create_todo.html', {'t':t, 's':s, 'form': form,'proje':proje})   

def todo_details(request, project_id, task_id, todo_id):
    t = Task.objects.filter(new_task=True)
    s = Suggestion.objects.filter(new_suggestion=True)
    todo = Todo.objects.get(pk=todo_id)
    return render(request, 'accounts/todo_details.html', {'t':t, 's':s, 'todo':todo})
...
def todo_finish(request, project_id, task_id, todo_id):
    ...
    if(todo.completed):
        return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
    return render(request, 'accounts/todo_finish.html', {'t':t, 's':s})
...
def jobs(request):
    ...
    for task in tasks1:
        if task.job == True:
            tasks.append(task)
    return render(request, 'accounts/jobs.html', {'tasks':tasks})

def create_job(request, task_id):
    task = Task.objects.get(pk=task_id)
    task.job = True
    task.save()
    return render(request, 'accounts/create_job.html', {'task':task})
...
def apply(request, task_id):
    ...
    if(user.is_authenticated):
        ...
        return render(request, 'accounts/apply.html', {'task':task, 'form':form})
    else:
        return redirect('/profiles/login')
...
def view_applications(request, task_id):
    ...
    return render(request, 'accounts/apps.html', {'task':task, 'app':app})
...
def decline(request, application_id, task_id):
    ...
    appl.delete()
    return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
    return render(request, 'accounts/decline.html', {})