...
from .forms import CreateProject,CreateFile,CreateSuggestion, CreateTask, CreateToDo, NewTask, CreateApplication, ProfileForm
...
def home(request):
	...
	for proj in projs:
		if request.user.profile in proj.workers.all():
	...
	for i in tasks:
		...
		if request.user in deps:
			task.append(i)
	context = {'projects_list': projects_list, 'suggestions':suggestions, 'task':task, 'apps':apps,'projs':projs}
...
def v_signup(request):
    ...
    if request.method == 'POST':
        ...
        form = SignUpForm()
    return render(request, 'viewersignup.html', {'form': form, 'form1':form1})
...
def notifications(request):
    ...
    files = File.objects.filter(new_suggestion=True)
    user = request.user
    if is_member(user):
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                for i in deps:
                    ...
                    if dep.username != project.deployer.username:
                        post_save.connect(my_handler, sender=user)
